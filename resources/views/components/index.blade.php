    ...
    <div class="pagina-ideia" id="pagina-ideia">
        ...
        <div class="container">
            ...
            <div class="row content-catalogo ">
                        ...
                        <div class="content-filtros comp-jddb0kzl" id="tab-0" style="display: none;">
                            @if (Request::segment(1) != 'produto' && Request::segment(1) != 'lojas')
                                <form id="form-busca" action="{{ route('index.pesquisa') }}" type="POST">
                                    <h1 class="tituloFiltros" style="color: #9e9e9e!important;">Catálogo de Produtos
                                    </h1>
                                    <div class="camposDeBusca">
                                        <div class="input-groupTata">
                                            <input type="text" class="campoGeral"
                                                value="{{ !empty($_GET['geral']) ? $_GET['geral'] : null }}"
                                                placeholder="Nome / Código / Descrição" id="geral" name="geral">
                                        </div>
                                        <div class="input-groupTata">
                                            <select type="text" class="campoGeral" id="marca" name="marca">
                                                ...
                                                @foreach ($data['marcas'] as $marcas)
                                                    <option
                                                        {{ !empty($_GET['marca']) && $_GET['marca'] == $marcas->nome ? 'selected' : '' }}
                                                        value="{{ $marcas->nome }}">{{ $marcas->nome }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="input-groupTata">
                                            <select onchange="grupoSelect($(this).val())" type="text"
                                                class="campoGeral" id="grupo" name="grupo">
                                                ...
                                                @foreach ($data['grupo'] as $grupo)
                                                    <option
                                                        {{ !empty($_GET['grupo']) && $_GET['grupo'] == $grupo->nome ? 'selected' : '' }}
                                                        value="{{ $grupo->nome }}">{{ $grupo->nome }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="input-groupTata">
                                            <select type="text" class="campoGeral" id="sub_grupo" name="sub_grupo">
                                                ...
                                                @php
                                                    $data['sub_grupo'] = SubGrupo::query()
                                                        ->orderBy('nome', 'asc')
                                                        ->get();
                                                @endphp
                                                @foreach ($data['sub_grupo'] as $sub_grupo)
                                                    <option
                                                        {{ !empty($_GET['sub_grupo']) && $_GET['sub_grupo'] == $sub_grupo->nome ? 'selected' : '' }}
                                                        value="{{ $sub_grupo->nome }}">{{ $sub_grupo->nome }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="">
                                        <button class="limpar" type="button" id="limpar-busca"
                                            onclick="limparBusca()">Limpar
                                            pesquisa
                                        </button>
                            ...
                            @if (Request::segment(1) == 'lojas')
                                <div class="col-12">
                                    ...
                                    <span class="" style="color: #8B0000">Venha fazer uma visita e tomar um
                                        cafézinho. Estamos prontos para te receber. </span>
                                </div>
                ...
                @endif
                <div class="@if (Request::segment(1) == 'produto') col-lg-12 @else col-lg-8 @endif">
                    {{ $slot }}
                </div>
            </div>
        </div>
                    ...
                    <div class="row">
                        <div class="col-12" style="text-align: center">
                            <div class="textoAssinatura" style="font-size: 14px!important;float: left">
                                ...
                                <a href="https://www.youtube.com/channel/UCAgCL7BetRdGcFEUq5AC22g" target="_blank"
                            ...
                            <div class="textoAssinatura" style="font-size: 14px!important;margin-top: 30px">
                                © Equipeças <span id="ideia-ano-footer"></span> - produzido orgulhosamente por nós
                                mesmos.
                            </div>
                        </div>
                        {{--                    <div class="col-6"> --}}
